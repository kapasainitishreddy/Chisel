...
let installed=false,visionPromise=null,landmarkerPromise=null,selectedFile=null;
...
async function getFaceLandmarker(){
  ...
  landmarkerPromise=(async()=>{
    ...
    try{return await FaceLandmarker.createFromOptions(files,{baseOptions,runningMode:'IMAGE',numFaces:1,outputFaceBlendshapes:false,outputFacialTransformationMatrixes:false});}
    catch{return FaceLandmarker.createFromOptions(files,{baseOptions:{...baseOptions,delegate:'CPU'},runningMode:'IMAGE',numFaces:1,outputFaceBlendshapes:false,outputFacialTransformationMatrixes:false});}
  })();
...
function renderResults(metrics){
  ...
  const headline=$('#csaCondition');headline.textContent=summary.headline;$('#csaConfidence').textContent=`${summary.confidence}%`;
...
function saveHistory(metrics){
  ...
  const compact={createdAt:new Date().toISOString(),confidence:metrics.confidence};for(const key of Object.keys(root.ChiselSkinAppearanceCore.METRIC_DEFS))compact[key]=metrics[key];
  history.unshift(compact);root.localStorage.setItem(HISTORY_KEY,JSON.stringify(history.slice(0,30)));
}
async function analyzeSelected(){
  const button=$('#csaAnalyze');if(!selectedFile||!button)return;button.disabled=true;button.setAttribute('aria-busy','true');status('Checking face position and lighting…');
  try{
    ...
    if(metrics.confidence<42)throw new Error('Lighting or exposure is too inconsistent for a useful appearance comparison. Retake in soft, even light.');
    drawRegions(ctx,rects);$('#csaPreviewWrap').hidden=false;renderResults(metrics);saveHistory(metrics);status('Appearance scan complete. Photo pixels stayed on this device; only numeric history is saved locally.');
