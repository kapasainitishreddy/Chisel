...
test('feature runtime is synchronized into the Android package assets', () => {
  assert.equal(
    hash('www/chisel-ar-coach-core.js'),
    hash('android/app/src/main/assets/public/chisel-ar-coach-core.js'),
    'feature runtime differs between canonical www and Android assets'
  );
});
