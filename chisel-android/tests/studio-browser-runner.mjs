...
try{
 ...
 check('expandedTrainerCatalog',await page.$$eval('#arCoachModal .ar-session',n=>n.length>=14&&n.every(x=>!x.hidden)));
 ...
 check('massageFilter',await page.$$eval('#arCoachModal .ar-session:not([hidden])',n=>n.length===7&&n.some(x=>x.textContent.includes('Jaw Relax'))));
 await page.keyboard.press('ArrowRight');
 check('keyboardJawNeckFilter',await page.$$eval('#arCoachModal .ar-session:not([hidden])',n=>n.length===4));
 await page.click('[data-cs-goal="all"]');
 check('allSessionsRestored',await page.$$eval('#arCoachModal .ar-session:not([hidden])',n=>n.length>=14));
