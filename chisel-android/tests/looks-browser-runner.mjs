...
let browser,page,interrupt=false,holdStatus=false,releaseStatus=null;const jobs=new Map();
...
try{
 ...
 page.on('request',async r=>{try{
  ...
  if(b.action==='status'){if(holdStatus)await new Promise(resolve=>{releaseStatus=resolve;});if(interrupt){interrupt=false;return r.abort('failed');}return r.respond({status:200,headers,body:JSON.stringify({status:'succeeded',jobId:b.requestId,imageUrl:'https://replicate.delivery/test-fixture.jpg',remaining:1})});}
 ...
 check('browseContainsRealCatalog',await page.$$eval('#clsPresets button',n=>n.length===25));await page.screenshot({path:out+'/browse-430.png'});check('singleSearchFocusBoundary',await page.evaluate(()=>getComputedStyle(document.getElementById('clsSearch')).outlineStyle==='none'&&getComputedStyle(document.querySelector('.cls-search')).outlineStyle!=='none'));
 ...
 // Backing out while the on-device check is awaiting a frame must not open consent later.
 await page.evaluate(()=>{window.__photoCheckResolved=false;ChiselSkinAppearance.detectFaces=()=>new Promise(resolve=>{window.__releasePhotoCheck=()=>{window.__photoCheckResolved=true;resolve([[]]);};});});
 await page.click('#clsGenerate');await page.waitForFunction(()=>typeof window.__releasePhotoCheck==='function');await page.click('#clsEditorClose');
 await page.evaluate(()=>window.__releasePhotoCheck());await page.waitForFunction(()=>window.__photoCheckResolved);await new Promise(r=>setTimeout(r,250));
 check('closedPreflightDoesNotOpenConsent',await page.evaluate(()=>!document.getElementById('clsConsent').open&&!document.getElementById('clsEditor').open));
 check('closedPreflightDoesNotUpload',report.calls.every(c=>c.action!=='create'));
 await style();await fixtureChecks();
 ...
 await page.click('#clsDismiss');
 // An already submitted job may finish, but it must not reopen a closed workspace.
 holdStatus=true;await page.click('#clsGenerate');await page.waitForSelector('#clsConsent[open]');await page.click('#clsConsentAccept');
 for(let i=0;i<100&&!releaseStatus;i++)await new Promise(r=>setTimeout(r,50));check('statusWaitReached',!!releaseStatus);
 await page.click('#clsEditorClose');const completedCreates=report.calls.filter(c=>c.action==='create').length;
 holdStatus=false;releaseStatus();await page.waitForFunction(()=>!document.getElementById('clsResult').hidden&&document.getElementById('clsAfter').naturalWidth>0,{timeout:15000});
 check('lateResultDoesNotReopenEditor',await page.$eval('#clsEditor',n=>!n.open));
 check('lateResultDoesNotDuplicateJob',report.calls.filter(c=>c.action==='create').length===completedCreates);
 await style();check('completedResultAvailableOnReturn',await page.$eval('#clsResult',n=>!n.hidden));await page.click('#clsDismiss');
 await page.setViewport({width:430,height:650,deviceScaleFactor:1});await page.$eval('#clsGenerate',n=>n.scrollIntoView({block:'end'}));
 check('shortScreenActionReachable',await page.$eval('#clsGenerate',n=>{const r=n.getBoundingClientRect();return r.top>=0&&r.bottom<=innerHeight;}));
 await page.setViewport({width:430,height:900,deviceScaleFactor:1});await page.click('#clsEditorClose');await page.evaluate(()=>go('home'));await page.click('.cps-avatar');await page.click('#cpsPhotoRemove');await page.waitForFunction(async()=>!(await ChiselLooksGallery.list()).length);check('photoRemovalClearsSavedCopies',await page.evaluate(async()=>!(await ChiselLooksGallery.list()).length));check('photoRemovalHidesPreview',await page.$eval('#clsResult',n=>n.hidden));
