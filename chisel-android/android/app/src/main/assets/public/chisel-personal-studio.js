...
(function(root,factory){
 ...
 function installTrainer(){
  ...
  featured.insertAdjacentHTML('beforeend',`<img class="cps-trainer-fallback" src="chisel-trainer-sculpture.webp" alt="" decoding="async"><div class="cps-edge-fade"></div><span class="cps-image-label" id="cpsTrainerMediaLabel">Movement preview</span><div class="cps-feature-caption"><span class="cps-feature-kicker">Today</span><h3 id="cpsSessionTitle"></h3><p class="cps-session-benefit" id="cpsSessionBenefit"></p><p id="cpsSessionMeta"></p><button type="button" id="cpsStartSession" class="cps-primary"><span>Start routine</span>${svg('chevron')}</button></div>`);
  body.prepend(featured);
  const library=el('section','cps-routine-library');library.setAttribute('aria-labelledby','cpsRoutineTitle');
