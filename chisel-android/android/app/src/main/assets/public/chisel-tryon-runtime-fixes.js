...
function installHairChoiceGuard(){if(typeof document==='undefined'||document.documentElement.dataset.chiselHairChoiceGuard==='1')return false;document.documentElement.dataset.chiselHairChoiceGuard='1';document.addEventListener('click',e=>{if(!e.target||!e.target.closest)return;const hair=e.target.closest('#hairChips .sc'),style=e.target.closest('#hairChips .sc,#beardChips .sc,#makeupChips .sc,#glassChips .sc');if(hair)hairChoiceLocked=true;if(style){setTimeout(syncPreviewSummary,0);setTimeout(syncPreviewSummary,180)}},true);return true}
function installPremiumHairRenderer(){
  if(typeof window==='undefined'||typeof window.drawHair!=='function')return false;if(window.drawHair.__chiselPremiumHairV4)return true;
  const premium=function(P,col,fH){
    ...
    const roots=raw.map((p,n)=>{const cw=1-Math.abs(n-mid)/mid,l=pr.hairlineLift+.038+cw*pr.frontLift*.08-(pr.fringe==='wispy'?cw*.010:0);return{x:p.x+ux*fH*l,y:p.y+uy*fH*l}});
    ...
    /* Quick AR is a restrained silhouette guide: roots start inside the existing hair mass, never across the forehead. */
...
function install(){
  ...
  if(typeof openStyle==='function'&&!openStyle.__chiselCleanDefaultV4){const o=openStyle,w=function(){hairChoiceLocked=false;const r=o.apply(this,arguments);try{styleBeard=0;if(typeof renderStyleChips==='function')renderStyleChips();syncPreviewSummary()}catch{}return r};w.__chiselCleanDefaultV4=true;openStyle=w}
