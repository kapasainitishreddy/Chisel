...
function createController({request,sleep=ms=>new Promise(r=>setTimeout(r,ms)),uuid=()=>crypto.randomUUID(),token=randomToken,persist=()=>{}}){
 ...
 async function poll(myEpoch){
  for(let i=0;i<90&&myEpoch===epoch;i++){
   ...
   const data=await request({action:'status',requestId:state.requestId,token:state.token,deviceId:state.deviceId});
 ...
 async function generate(input){
  ...
  state={status:'submitting',requestId:uuid(),token:token(),deviceId:input.deviceId,look:input.look,createdAt:Date.now()};const creds={requestId:state.requestId,token:state.token,deviceId:state.deviceId};emit({});
 ...
 async function resume(saved){
  ...
  try{const data=await request({action:'status',requestId:state.requestId,token:state.token,deviceId:state.deviceId});if(mine===epoch&&!accept(data))await poll(mine);}
  catch(error){if(mine===epoch)emit({status:'interrupted',error:error.message||'network_error'});}
  finally{if(mine===epoch)locked=false;}return snapshot();
 }
 async function cancel(){
  const requestData={action:'cancel',requestId:state.requestId,token:state.token,deviceId:state.deviceId};epoch++;locked=false;emit({status:'canceled',imageUrl:null,error:null});
